...
CREATE DATABASE IF NOT EXISTS `ca_local`.`tipo_user`(
	`id_data_user` INT(11) PRIMARY KEY AUTO_INCREMENT,
	'name_tipo_user' varchar(25) NOT NULL
);

INSERT INTO `ca_local`.`tipo_user`(`id_data_user`,`name_tipo_user`)VALUES
(NULL,'ADMINISTRADOR'),
(NULL,'SUPERVISOR'),
(NULL,'GERENTE'),
(NULL,'EMPLEADO');
...
CREATE DATABASE IF NOT EXISTS `ca_local`.`tipo_reportes`(
	`id_tipo_reporte` INT(11) PRIMARY KEY AUTO_INCREMENT
);

CREATE DATABASE IF NOT EXISTS `ca_local`.`reportes`(
