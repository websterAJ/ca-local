...
if (conexion()) :
	if (isset($_POST)) :
		...
		$array_data["cedula"]=filter_var($data["cedula"],FILTER_SANITIZE_STRING);
		...
		if(empty($array_data["cedula"])&&empty($array_data["nombre"])&&empty($array_data["apellido"])&&empty($array_data["correo"])&&empty($array_data["telefono"])&&empty($array_data["id_tip_per"])&&empty($array_data["id_dep"])):
?>
